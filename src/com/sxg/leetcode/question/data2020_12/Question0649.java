    ...
    public String predictPartyVictory(String senate) {
        ...
        for (int index = 0; index < length; index++) {
            if (senate.charAt(index) == 'R') {
                dequeR.addLast(index);
            } else {
                dequeD.addLast(index);
            }
        }

        while (!dequeD.isEmpty() && !dequeR.isEmpty()) {
            int indexD = dequeD.pollFirst();
            int indexR = dequeR.pollFirst();

            if (indexD < indexR) {
                dequeD.addLast(indexD + length);
            } else {
                dequeR.addLast(indexR + length);
            }
        }
